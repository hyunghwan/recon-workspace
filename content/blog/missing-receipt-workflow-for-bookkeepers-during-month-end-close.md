---
slug: missing-receipt-workflow-for-bookkeepers-during-month-end-close
title: Missing receipt workflow for bookkeepers during month-end close
description: Build a missing receipt workflow that keeps requests attached to transactions, reduces duplicate follow-up, and helps bookkeepers move month-end close faster.
excerpt: Missing receipts become expensive when requests, notes, and transaction review drift apart. This guide shows bookkeepers how to build a cleaner follow-up workflow.
...
You can see this logic reflected in the [sample workspace](/app), where the unresolved queue is built around the next action needed instead of a flat list of vague exceptions.
...
That is why internal linking is important. Linking to the [product site](/) gives the reader the category context, while linking to the [sample workspace](/app) gives them a low-friction way to see the workflow in action. The article does not need to over-sell. It just needs to close the gap between pain and next step.
