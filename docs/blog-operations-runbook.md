...
6. Run `SITE_ORIGIN=https://your-domain.com pnpm build`.
...
```bash
pnpm blog:check
SITE_ORIGIN=https://your-domain.com pnpm build
pnpm preview
```

## Production rule for absolute URLs
- Never hardcode the public domain in article content or templates.
- Use `SITE_ORIGIN` at build time for canonical URLs, Open Graph tags, RSS, and sitemap output.
